    ...
    def printer(self):
        if not self.no_banner:
            print("{}\n{}\n{}".format("-" * self.BANNER_WIDTH,
                "garleak' Bot Detective at work ...".center(self.BANNER_WIDTH),
                "-" * self.BANNER_WIDTH))
...
def main():
    """ This is the function that is run from commandline with `garleak` """
    args = get_args_parser().parse_args()
    gl = GarLeak(args.__dict__)
    try:
        gl.run()
    except KeyboardInterrupt:
        print('garleak user interupted')
    gl.printer()
